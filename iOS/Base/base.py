...
def video_capture(cp_method, btn_rec, duration = 5):
    ...
    try:
        ...
        time.sleep(1)
        sc.driver.find_element_by_name(iOS_elements.el_cam_next).click()
...
def clip_reverse():
    ...
    try:
        WebDriverWait(sc.driver, 180, 1).until(
...
def clip_mult_select():
    ...
    sc.logger.info('删除镜头')
    WebDriverWait(sc.driver, 5, 1).until(
        lambda x: x.find_element_by_name("删除")).click()
...
def fx_add():
    ...
    try:
        sc.logger.info('下载一个"特效"')
        sc.driver.find_element_by_xpath(iOS_elements.el_fx_download).click()
        time.sleep(5)
    except NoSuchElementException:
        sc.logger.info('当前页面已无未下载"特效"')
